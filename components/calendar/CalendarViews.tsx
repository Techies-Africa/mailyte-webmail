    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="grid grid-cols-7 border-b border-neutral-200 dark:border-neutral-800">
        {days.slice(0, 7).map((day) => (
          <div
            key={day.toISOString()}
            className="px-1 py-2 text-[10px] font-medium uppercase tracking-wide text-neutral-500 dark:text-neutral-400 sm:px-2 sm:text-xs"
          >
            {format(day, 'EEE')}
          </div>
        ))}
      ...
      {/* Rows share the height, but never below 5.5rem: on a short screen
          (a phone in landscape) the weeks scroll rather than squeeze their
          chips into each other. */}
      <div className="grid min-h-0 flex-1 auto-rows-[minmax(5.5rem,1fr)] grid-cols-7 overflow-y-auto">
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={[
                'min-w-0 overflow-hidden border-b border-r border-neutral-200 p-1 dark:border-neutral-800',
              ...
              <div className="space-y-0.5">
                {dayEvents.slice(0, 3).map((event) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-1 truncate rounded px-0.5 py-0.5 text-left text-[10px] text-white sm:px-1 sm:text-[11px] ${colourFor(event)}`}
                    title={event.summary ?? 'Untitled'}
                  >
                    {/* No time on a phone: in a 50px day it filled the chip
                        and the title never showed. */}
                    {!event.all_day && (
